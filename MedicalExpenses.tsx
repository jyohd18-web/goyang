...
import { Plus, Trash2, Calendar, Hospital, Stethoscope, Camera, Search, ChevronDown, ChevronUp, Edit2 } from 'lucide-react';

interface Props {
  items: MedicalExpense[];
  onAdd: (expense: Omit<MedicalExpense, 'id'>) => void;
  onUpdate: (id: string, expense: Omit<MedicalExpense, 'id'>) => void;
  onRemove: (id: string) => void;
}

const MedicalExpenses: React.FC<Props> = ({ items, onAdd, onUpdate, onRemove }) => {
  ...
  const handleEdit = (item: MedicalExpense) => {
      ...
      <div className="space-y-3">
        <div className="flex flex-col md:flex-row gap-3">
          <div className="flex-1 relative group">
            ...
            />
            <button onClick={() => setIsSearchDetailed(!isSearchDetailed)} className="absolute right-2 top-1/2 -translate-y-1/2 p-2 hover:bg-indigo-50 rounded-xl text-gray-400">
        ...
        {isSearchDetailed && (
          <div className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 grid grid-cols-1 md:grid-cols-3 gap-4 animate-in slide-in-from-top-2 duration-200">
            <div className="space-y-1">
              ...
              <input type="text" placeholder="병원명 입력" value={searchFilters.clinicName} onChange={(e) => setSearchFilters({...searchFilters, clinicName: e.target.value})} className="w-full px-4 py-2 bg-gray-50 border-none rounded-xl text-sm focus:ring-1 focus:ring-indigo-300" />
            </div>
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">진료항목</label>
              <input type="text" placeholder="진료/수술명 입력" value={searchFilters.diagnosisName} onChange={(e) => setSearchFilters({...searchFilters, diagnosisName: e.target.value})} className="w-full px-4 py-2 bg-gray-50 border-none rounded-xl text-sm focus:ring-1 focus:ring-indigo-300" />
            </div>
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">방문 날짜</label>
              <input type="date" value={searchFilters.date} onChange={(e) => setSearchFilters({...searchFilters, date: e.target.value})} className="w-full px-4 py-2 bg-gray-50 border-none rounded-xl text-sm focus:ring-1 focus:ring-indigo-300" />
            </div>
            <div className="md:col-span-3 flex justify-end">
              <button onClick={() => { setSearchFilters({ clinicName: '', diagnosisName: '', date: '' }); setSearchQuery(''); }} className="text-xs text-indigo-500 hover:text-indigo-800 flex items-center gap-1 font-medium">필터 초기화</button>
            </div>
          </div>
        )}
      </div>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2 space-y-1">
              ...
              <div className="relative">
                ...
                <label htmlFor="receipt-upload" className="w-full flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-2xl p-6 hover:bg-indigo-50 transition-all cursor-pointer group">
            ...
            <div className="md:col-span-2 flex gap-3 mt-4">
              <button type="submit" className="flex-1 bg-indigo-600 text-white py-4 rounded-2xl font-bold hover:bg-indigo-700 transition-all shadow-md active:scale-[0.98]">
                {editingId ? '진료 내역 수정' : '진료 내역 저장'}
              </button>
              <button type="button" onClick={closeForm} className="flex-1 bg-gray-100 text-gray-600 py-4 rounded-2xl font-bold hover:bg-gray-200 transition-all">취소</button>
            </div>
          </form>
        </div>
      )}
            ...
            <div key={item.id} className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 flex items-start justify-between group transition-all hover:shadow-md overflow-hidden">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                  <button onClick={() => handleEdit(item)} className="text-slate-400 hover:text-slate-800 p-2 rounded-lg hover:bg-slate-50">
                    <Edit2 size={18} />
                  </button>
                  <button onClick={() => onRemove(item.id)} className="text-slate-300 hover:text-red-500 p-2 rounded-lg hover:bg-red-50">
                    <Trash2 size={18} />
                  </button>
                </div>
              </div>
