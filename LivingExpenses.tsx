...
import { Plus, Trash2, Calendar, Store, Tag, Package, Truck, Search, ChevronDown, ChevronUp, Edit2 } from 'lucide-react';

interface Props {
  items: LivingExpense[];
  onAdd: (expense: Omit<LivingExpense, 'id'>) => void;
  onUpdate: (id: string, expense: Omit<LivingExpense, 'id'>) => void;
  onRemove: (id: string) => void;
}

const LivingExpenses: React.FC<Props> = ({ items, onAdd, onUpdate, onRemove }) => {
  ...
  const handleEdit = (item: LivingExpense) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId) {
      onUpdate(editingId, { ...formData, totalPrice });
    } else {
        ...
        {isSearchDetailed && (
          <div className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 animate-in slide-in-from-top-2 duration-200">
            <div className="space-y-1">
              ...
              <input type="text" placeholder="물품명 입력" value={searchFilters.itemName} onChange={(e) => setSearchFilters({...searchFilters, itemName: e.target.value})} className="w-full px-4 py-2 bg-gray-50 border-none rounded-xl text-sm focus:ring-1 focus:ring-slate-300" />
            </div>
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">판매처</label>
              <input type="text" placeholder="판매처 입력" value={searchFilters.vendor} onChange={(e) => setSearchFilters({...searchFilters, vendor: e.target.value})} className="w-full px-4 py-2 bg-gray-50 border-none rounded-xl text-sm focus:ring-1 focus:ring-slate-300" />
            </div>
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">카테고리</label>
              <select value={searchFilters.category} onChange={(e) => setSearchFilters({...searchFilters, category: e.target.value as Category | ''})} className="w-full px-4 py-2 bg-gray-50 border-none rounded-xl text-sm focus:ring-1 focus:ring-slate-300 appearance-none">
            ...
            <div className="space-y-1">
              ...
              <input type="date" value={searchFilters.date} onChange={(e) => setSearchFilters({...searchFilters, date: e.target.value})} className="w-full px-4 py-2 bg-gray-50 border-none rounded-xl text-sm focus:ring-1 focus:ring-slate-300" />
            </div>
            <div className="lg:col-span-4 flex justify-end">
              <button onClick={() => { setSearchFilters({ itemName: '', vendor: '', category: '', date: '' }); setSearchQuery(''); }} className="text-xs text-slate-500 hover:text-slate-800 flex items-center gap-1 font-medium">필터 초기화</button>
            </div>
          </div>
        )}
      </div>
        ...
        <div className="bg-white rounded-3xl p-6 shadow-xl border border-gray-100 border-t-4 border-t-slate-800 animate-in slide-in-from-top-4 duration-300">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2 flex gap-3 mt-4">
              <button type="submit" className="flex-1 bg-slate-800 text-white py-4 rounded-2xl font-bold hover:bg-black transition-all">
