...
import { ChevronRight, CreditCard, HeartPulse, Sparkles, ShoppingCart, PiggyBank } from 'lucide-react';
...
const Summary: React.FC<SummaryProps> = ({ state, onNavigate }) => {
  ...
  const savingsTotal = state.savings.reduce((sum, item) => sum + item.amount, 0);
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <header className="mb-8 flex justify-between items-start">
        ...
        <div className="bg-emerald-50 px-4 py-2 rounded-2xl border border-emerald-100 flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] text-emerald-500 font-bold uppercase leading-none">비상금 적금액</p>
            <p className="text-lg font-black text-emerald-700 leading-tight">{formatKRW(savingsTotal)}</p>
          </div>
        </div>
      </header>
