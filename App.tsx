...
import { 
  ...
  TrendingUp,
  BarChart3,
  PiggyBank // Wallet 대신 PiggyBank 아이콘 사용
} from 'lucide-react';
import { ActiveTab, AppState, LivingExpense, MedicalExpense, InitialCost, Savings } from './types';
...
import SavingsRecords from './components/SavingsRecords'; // IncomeRecords를 SavingsRecords로 변경하여 사용
...
const App: React.FC = () => {
  ...
  const [state, setState] = useState<AppState>(() => {
    ...
    if (saved) {
      ...
      return {
        ...
        medicalExpenses: parsed.medicalExpenses || [],
        initialCosts: parsed.initialCosts || [],
        savings: parsed.savings || parsed.incomes || [] // 하위 호환성을 위해 incomes도 체크
      };
    }
    return {
      livingExpenses: [],
  ...
  const updateLivingExpense = (id: string, updated: Omit<LivingExpense, 'id'>) => {
    setState(prev => ({
      ...prev,
      livingExpenses: prev.livingExpenses.map(item => item.id === id ? { ...updated, id } : item)
    }));
  };
  ...
  const updateMedicalExpense = (id: string, updated: Omit<MedicalExpense, 'id'>) => {
    setState(prev => ({
      ...prev,
      medicalExpenses: prev.medicalExpenses.map(item => item.id === id ? { ...updated, id } : item)
    }));
  };
  ...
  // 고양이 적금 핸들러
  const addSavings = (record: Omit<Savings, 'id'>) => {
    const newRecord = { ...record, id: crypto.randomUUID() };
    setState(prev => ({ ...prev, savings: [newRecord, ...prev.savings] }));
  };

  const removeSavings = (id: string) => {
    setState(prev => ({ ...prev, savings: prev.savings.filter(i => i.id !== id) }));
  };
    ...
    switch (activeTab) {
      ...
      case 'living':
        return <LivingExpenses 
          items={state.livingExpenses} 
          onAdd={addLivingExpense} 
          onUpdate={updateLivingExpense}
          onRemove={removeLivingExpense} 
        />;
      case 'medical':
        return <MedicalExpenses 
          items={state.medicalExpenses} 
          onAdd={addMedicalExpense} 
          onUpdate={updateMedicalExpense}
          onRemove={removeMedicalExpense} 
        />;
      case 'initial':
        return <InitialCosts items={state.initialCosts} onAdd={addInitialCost} onRemove={removeInitialCost} />;
      case 'savings':
        return <SavingsRecords items={state.savings} onAdd={addSavings} onRemove={removeSavings} />;
      default:
        return <Summary state={state} onNavigate={setActiveTab} />;
    }
  };
  ...
  return (
    <div className="min-h-screen pb-24 lg:pb-0 lg:pl-64 bg-gray-50 text-slate-900">
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex flex-col w-64 fixed left-0 top-0 bottom-0 bg-white border-r border-gray-200 z-50">
        ...
        <nav className="flex-1 px-4 space-y-1 overflow-y-auto">
          ...
          <NavBtn active={activeTab === 'savings'} onClick={() => setActiveTab('savings')} icon={<PiggyBank size={20} />} label="고양이 적금" />
        </nav>
      </aside>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex items-center justify-around py-3 px-2 z-50 shadow-[0_-2px_10px_rgba(0,0,0,0.05)] overflow-x-auto">
        ...
        <MobNavBtn active={activeTab === 'savings'} onClick={() => setActiveTab('savings')} icon={<PiggyBank size={24} />} label="적금" />
